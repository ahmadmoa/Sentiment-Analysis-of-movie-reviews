...
def preprocessing(X_train):
    # Ensure proper text processing before tokenization
    X_train = X_train.apply(remove_html_tags)
    X_train = X_train.apply(convert_to_lowercase)
    X_train = X_train.apply(remove_punctuation)
    # X_train = X_train.apply(remove_non_ascii)  # Remove non-ASCII characters
    X_train = X_train.apply(tokenize)  # Lists of words
    X_train = X_train.apply(remove_stop_words)  # Lists of words
    X_train = X_train.apply(stem)  # Lists of stemmed words
    X_train = X_train.apply(join_tokens)  # Back to strings
    return X_train
